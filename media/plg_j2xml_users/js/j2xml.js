...
eshiol.j2xml.users = {};
eshiol.j2xml.users.version = '__DEPLOY_VERSION__';
eshiol.j2xml.users.requires = '22.12.373';
eshiol.j2xml.users.requiresj2XML = '3.9.222';
...
eshiol.j2xml.convert
		.push(function(xml) {
			...
			if (versionCompare(eshiol.j2xml.version, eshiol.j2xml.users.requires) < 0) {
				eshiol.renderMessages({
					'error' : [ 'J2XML - Users Importer v'
							...
							+ ' requires J2XML ' + eshiol.j2xml.users.requiresj2XML ]
				});
				return false;
			}
			...
			for (var i = 1; i < csv.length; i++) {
				...
				x += "\t\t<registerDate><![CDATA[" + (new Date().toISOString().slice(0, 19).replace('T', ' '))
						+ "]]></registerDate>\n";
